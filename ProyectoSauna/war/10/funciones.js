...
onload=function() 
{
	...
	urlDestino="www.google.com";
	
	claseNormal="input";
	claseError="inputError";
	
	ayuda=new Array();
	ayuda["Nombre"]="Ingresa un nombre";
	ayuda["Descripcion"]="Ingresa la descripcion del cuarto";
	ayuda["numero"]="Ingresa el numro de habitacion";
	ayuda["foto"]="Selecciona una foto";
...
function validaForm()
{
	...
	var numero=eliminaEspacios(form.inputNumero.value);
	var descripcion=eliminaEspacios(form.inputDescripcion.value);
	var foto=eliminaEspacios(form.inputFoto.value);
	
	
	if(!validaLongitud(nombre, 0, 4, 50)) campoError(form.inputNombre);
	if(!validaLongitud(numero, 1, 4, 50)) campoError(form.inputEmpresa);
	if(!validaLongitud(descripcion, 1, 4, 50)) campoError(form.inputDescripcion);
	if(!validaLongitud(foto, 0, 5, 500)) campoError(form.inputFoto);
	...
	else
	{
		...
		ajax.send("nombre="+nombre+"&empresa="+numero+"&telefono="+desripcion+"&descripcion="+foto");
